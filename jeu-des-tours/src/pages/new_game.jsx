...
import * as styles from "./newGame.css";
    ...
    function fetchGameId() {
        fetch('http://127.0.0.1:3000/api/tower-game/get-id')
            ...
            .then((data) => {
                ...
                if (data.gameId === "") {
                    console.error('Erreur lors de la récupération de l\'ID');
                } else {
                    window.location.href = `/gamepage?id=${data.gameId}`;

                }
            })
            .catch((error) => console.error('Erreur fetchGameId:', error));
    }

    useEffect(() => {
        const interval = setInterval(fetchGameId, 1000);
        console.log('ID récupéré avec succès :', gameId);
