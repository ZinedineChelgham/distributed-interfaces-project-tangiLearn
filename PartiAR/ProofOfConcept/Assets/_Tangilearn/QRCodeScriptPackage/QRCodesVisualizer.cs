...
namespace Microsoft.MixedReality.SampleQRCodes
{
    public class QRCodesVisualizer : MonoBehaviour
    {
        ...
        public GameObject prefabMenuQRCode;
        public System.Guid updatedQR;
            ...
            lock (pendingActions)
            {
                while (pendingActions.Count > 0)
                {
                    ...
                    if (action.type == ActionData.Type.Added)
                    {
                        ...
                        qrCodeObject.transform.GetChild(0).gameObject.SetActive(false);
                    }
                    else if (action.type == ActionData.Type.Updated)
                    {
                        updatedQR = action.qrCode.Id;
                        DateTime date_time = DateTime.Now;
                        lastTimeUpdated = date_time.Millisecond;
                        if (!qrCodesObjectsList.ContainsKey(action.qrCode.Id))
                        {
                            Debug.Log("CONTIENT PAS CE QR");
                            GameObject qrCodeObject = Instantiate(qrCodePrefab, new Vector3(0, 0, 0), Quaternion.identity);
                            qrCodeObject.GetComponent<SpatialGraphNodeTracker>().Id = action.qrCode.SpatialGraphNodeId;
                            qrCodeObject.GetComponent<QRCode>().qrCode = action.qrCode;
                            qrCodesObjectsList.Add(action.qrCode.Id, qrCodeObject);
